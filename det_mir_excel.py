...
count = 1
while count <= 278:
    url = f'https://www.detmir.ru/catalog/index/name/myagkie_igrushki/?page={count}'
    headers = {
    ...
    category = block.find('h1', {'class': 'E_1'}).text.strip()
    print(category)
    ...
    for head in heads:
        ...
        if os.path.exists(excel_file):
            # Читаем существующий файл Excel
            df_existing = pd.read_excel(excel_file)
            # Преобразуем его в DataFrame
            df = pd.DataFrame([storage])  # Преобразуем новый словарь в DataFrame
            df_combined = pd.concat([df_existing, df], ignore_index=True)  # Объединяем старые и новые данные
        else:
            # Если файл не существует, создаем новый DataFrame
            df_combined = pd.DataFrame([storage])

        # Записываем объединенные данные обратно в файл Excel
